...
alias build_cygnus_all="cd ${ZMK_HOME} && ${build_cygnus_left} && ${archive_cygnus_left} && ${build_cygnus_right} && ${archive_cygnus_right} && cd ${KEYBOARD_HOME}"
alias build_cygnus="cd ${ZMK_HOME} && ${build_cygnus_left} && ${archive_cygnus_left} && cd ${KEYBOARD_HOME}"

echo "Creating Diamond alias..."
build_diamond_dongle="west build -s app -b nice_nano_v2 --build-dir build/diamond_dongle -- -DSHIELD='diamond_dongle dongle_display' -DZMK_CONFIG=$KEYBOARD_HOME/src/zmk/boards/handwired -DZMK_EXTRA_MODULES=$DONGLE_HOME"
build_diamond_left="west build -s app -b nice_nano_v2 --build-dir build/diamond_left -- -DSHIELD='diamond_left' -DZMK_CONFIG=$KEYBOARD_HOME/src/zmk/boards/handwired"
build_diamond_right="west build -s app -b nice_nano_v2 --build-dir build/diamond_right -- -DSHIELD='diamond_right' -DZMK_CONFIG=$KEYBOARD_HOME/src/zmk/boards/handwired"
archive_diamond_dongle="mkdir -p $KEYBOARD_HOME/build/artifacts; [ -f build/diamond_dongle/zephyr/zmk.uf2 ] && mv build/diamond_dongle/zephyr/zmk.uf2 $KEYBOARD_HOME/build/artifacts/diamond_dongle-zmk.uf2"
archive_diamond_left="mkdir -p $KEYBOARD_HOME/build/artifacts; [ -f build/diamond_left/zephyr/zmk.uf2 ] && mv build/diamond_left/zephyr/zmk.uf2 $KEYBOARD_HOME/build/artifacts/diamond_left-zmk.uf2"
archive_diamond_right="mkdir -p $KEYBOARD_HOME/build/artifacts; [ -f build/diamond_right/zephyr/zmk.uf2 ] && mv build/diamond_right/zephyr/zmk.uf2 $KEYBOARD_HOME/build/artifacts/diamond_right-zmk.uf2"
alias build_diamond_all="cd ${ZMK_HOME} && ${build_diamond_dongle} && ${archive_diamond_dongle} && ${build_diamond_left} && ${archive_diamond_left} && ${build_diamond_right} && ${archive_diamond_right} && cd ${KEYBOARD_HOME}"
alias build_diamond="cd ${ZMK_HOME} && ${build_diamond_dongle} && ${archive_diamond_dongle} && cd $KEYBOARD_HOME"
