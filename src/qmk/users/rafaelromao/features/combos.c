...
combo_t key_combos[] = {
    ...
    COMBO(l_low_par_combo, TD_PARN),  COMBO(l_rai_f13_combo, KC_F13),     COMBO(l_rai_f14_combo, KC_F14),
    COMBO(l_rai_f15_combo, KC_F15),   COMBO(secret_1_combo, MC_SECRET_1),
#endif

    COMBO(l_acc_esc_combo, MC_ESCC),  COMBO(l_nav_esc_combo, MC_ESCC),    COMBO(l_num_esc_combo, MC_ESCC),
    COMBO(l_num_num_combo, TO_A1),    COMBO(l_num_f10_combo, KC_F10),     COMBO(l_num_f11_combo, KC_F11),
    COMBO(l_num_sav_combo, MC_SAVE),  COMBO(l_rom_esc_combo, MC_ESCC),    COMBO(l_rom_esv_combo, MC_ESAV),
    COMBO(l_rom_fin_combo, MC_FIND),  COMBO(l_rom_ful_combo, MC_FULL),    COMBO(l_rom_j_combo, KC_J),
    COMBO(l_rom_k_combo, KC_K),       COMBO(l_rom_cap_combo, TG_CAPS),    COMBO(l_rom_num_combo, TG_NUMC),
    COMBO(l_rom_q_combo, KC_Q),       COMBO(l_rom_v_combo, KC_V),         COMBO(l_rom_sav_combo, MC_SAVE),
    COMBO(l_rom_und_combo, MC_UNDO),  COMBO(l_rom_usc_combo, KC_UNDS),    COMBO(l_rom_w_combo, KC_W),
    COMBO(l_rom_z_combo, KC_Z),       COMBO(r_acc_ao_combo, MC_AO),       COMBO(r_acc_bsp_combo, KC_BSPC),
    COMBO(r_acc_cao_combo, MC_CAO),   COMBO(r_acc_coes_combo, MC_COES),   COMBO(r_acc_ja_combo, MC_JA),
    COMBO(l_acc_lh_combo, MC_LH),     COMBO(l_acc_cir_combo, MC_CIRC),    COMBO(r_acc_til_combo, MC_TILD),
    COMBO(r_acc_oe_combo, MC_OE),     COMBO(r_acc_squo_combo, MC_SQUO),   COMBO(l_acc_dquo_combo, MC_DQUO),
    COMBO(l_acc_bti_combo, MC_BTIC),  COMBO(r_acc_dol_combo, KC_DLR),     COMBO(r_low_ent_combo, KC_ENT),
    COMBO(r_low_sco_combo, TD_SCLE),  COMBO(r_sho_clo_combo, MC_CLOS),    COMBO(l_sho_bui_combo, MC_BUID),
    COMBO(l_sho_ref_combo, MC_REFC),  COMBO(r_sho_z0r_combo, MC_Z0R),     COMBO(r_med_ent_combo, TD_EENT),
    COMBO(r_nav_bsp_combo, KC_BSPC),  COMBO(r_nav_ent_combo, KC_ENT),     COMBO(r_nav_cop_combo, MC_COPY),
    COMBO(r_nav_pas_combo, MC_PAST),  COMBO(r_num_bsp_combo, KC_BSPC),    COMBO(r_num_ent_combo, KC_ENT),
    COMBO(r_num_npd_combo, TG_NUM),   COMBO(r_num_tab_combo, KC_TAB),     COMBO(r_rai_bsp_combo, KC_BSPC),
    ...
    COMBO(r_rai_ltgt_combo, MC_LTGT), COMBO(r_rai_neq_combo, MC_NEQ),     COMBO(r_rai_sar_combo, TD_SARR),
    COMBO(r_rom_bsp_combo, KC_BSPC),  COMBO(r_rom_cop_combo, MC_COPY),    COMBO(r_rom_d_combo, KC_D),
    COMBO(r_rom_ent_combo, KC_ENT),   COMBO(r_rom_npd_combo, TG_NUM),     COMBO(r_rom_p_combo, KC_P),
