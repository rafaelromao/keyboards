...
const uint16_t PROGMEM l_nav_ent_combo[] = {RAI_ACT, KC_DOWN, KC_UP, COMBO_END};
...
const uint16_t PROGMEM r_rom_sce_combo[] = {LALTT_H, TD_COMM, TD_DOT, COMBO_END};
...
combo_t key_combos[] = {
    ...
    COMBO(l_nav_ent_combo, KC_ENT),    COMBO(l_nav_esv_combo, MC_ESAV),  COMBO(l_num_sav_combo, MC_SAVE),
    COMBO(r_nav_bsp_combo, KC_BSPC),   COMBO(r_rai_bsp_combo, KC_BSPC),  COMBO(r_num_bsp_combo, KC_BSPC),
    COMBO(r_low_bsp_combo, KC_BSPC),   COMBO(r_acc_bsp_combo, KC_BSPC),  COMBO(r_num_ent_combo, KC_ENT),
    COMBO(r_acc_squo_combo, MC_SQUO),  COMBO(l_low_par_combo, TD_PARN),  COMBO(r_low_ent_combo, KC_ENT),
    COMBO(r_acc_ae_combo, MC_AE),      COMBO(r_acc_ao_combo, MC_AO),     COMBO(r_acc_cao_combo, MC_CAO),
    COMBO(r_acc_oe_combo, MC_OE),      COMBO(r_acc_coes_combo, MC_COES), COMBO(r_mac_z0r_combo, MC_Z0R),
    COMBO(r_mac_clo_combo, MC_CLOS),   COMBO(r_rom_cop_combo, MC_COPY),  COMBO(r_rom_scp_combo, MC_SCOP),
    COMBO(r_rom_sco_combo, TD_SCLE),   COMBO(l_rom_usc_combo, KC_UNDS),  COMBO(l_rom_ful_combo, MC_FULL),
    COMBO(r_rom_p_combo, KC_P),        COMBO(r_rom_pas_combo, MC_PAST),  COMBO(r_rom_spa_combo, MC_SPAS),
    ...
    COMBO(r_rom_soe_combo, MC_SOES),   COMBO(r_rom_bsp_combo, KC_BSPC),  COMBO(r_rom_ent_combo, KC_ENT),
    COMBO(r_rom_sal_combo, MC_SELC),   COMBO(r_rai_sar_combo, TD_SARR),  COMBO(r_rai_deq_combo, MC_DEQ),
    COMBO(r_rai_neq_combo, MC_NEQ),    COMBO(r_rai_ltgt_combo, MC_LTGT), COMBO(r_rai_lt_combo, TD_LT),
    COMBO(r_rai_gt_combo, TD_GT),      COMBO(r_rai_cur_combo, MC_CUR),   COMBO(r_med_ent_combo, TD_EENT),
