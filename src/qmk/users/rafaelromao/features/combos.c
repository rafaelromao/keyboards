...
const uint16_t PROGMEM l_low_sqo_combo[] = {TD_CPAR, TD_DQUO, COMBO_END};
const uint16_t PROGMEM l_low_bti_combo[] = {TD_CBR, TD_QUES, COMBO_END};
...
const uint16_t PROGMEM r_nav_pdn_combo[] = {KC_LEFT, KC_DOWN, COMBO_END};
const uint16_t PROGMEM r_nav_pup_combo[] = {KC_DOWN, KC_UP, COMBO_END};
const uint16_t PROGMEM r_nav_bsp_combo[] = {KC_ENT, KC_TAB, COMBO_END};
...
const uint16_t PROGMEM l_sym_sec_combo[]  = {KC_RCBR, KC_HASH, COMBO_END};
...
const uint16_t PROGMEM r_sym_circ_combo[] = {KC_MINS, KC_PLUS, COMBO_END};
const uint16_t PROGMEM r_sym_til_combo[]  = {KC_EXLM, KC_AMPR, COMBO_END};
const uint16_t PROGMEM r_sym_dol_combo[]  = {KC_PLUS, KC_SLSH, COMBO_END};
const uint16_t PROGMEM r_sym_bsl_combo[]  = {KC_AMPR, KC_PIPE, COMBO_END};
...
const uint16_t PROGMEM r_med_esp_combo[]  = {HYPR_R, KC_VOLD, KC_VOLU, COMBO_END};
...
combo_t key_combos[] = {
    ...
    COMBO(r_num_pip_combo, KC_PIPE),    COMBO(r_num_zz_combo, MC_ZZ),     COMBO(l_sym_sec_combo, MC_SEC),
    ...
    COMBO(r_sym_circ_combo, KC_CIRC),   COMBO(r_sym_dol_combo, KC_DLR),   COMBO(r_sym_til_combo, KC_TILD),
    COMBO(r_sym_bsl_combo, KC_BSLS),    COMBO(r_sym_bsp_combo, KC_BSPC),  COMBO(r_low_com_combo, KC_COMM),
    COMBO(r_num_com_combo, KC_COMM),    COMBO(l_acc_esc_combo, MC_ESCC),  COMBO(l_acc_und_combo, KC_UNDS),
    COMBO(l_acc_ques_combo, MC_QUES),   COMBO(l_acc_hash_combo, KC_HASH), COMBO(l_low_cur_combo, MC_CUR),
    COMBO(l_low_bra_combo, MC_BRAC),    COMBO(l_low_par_combo, TD_PARN),  COMBO(l_nav_esc_combo, MC_ESCC),
    COMBO(l_num_esc_combo, MC_ESCC),    COMBO(l_num_f10_combo, KC_F10),   COMBO(l_num_f11_combo, KC_F11),
    COMBO(l_num_ent_combo, KC_ENT),     COMBO(l_num_num_combo, TO_A1),    COMBO(l_num_sav_combo, MC_SAVE),
    ...
    COMBO(l_rom_sav_combo, MC_SAVE),    COMBO(l_rom_und_combo, MC_UNDO),  COMBO(l_rom_v_combo, KC_V),
    COMBO(l_rom_w_combo, KC_W),         COMBO(l_acc_lpa_combo, KC_LPRN),  COMBO(l_acc_rpa_combo, KC_RPRN),
    COMBO(l_sho_bui_combo, MC_BUID),    COMBO(l_sho_ref_combo, MC_REFC),  COMBO(r_acc_ao_combo, MC_AO),
    COMBO(r_acc_bsp_combo, KC_BSPC),    COMBO(r_acc_cao_combo, MC_CAO),   COMBO(r_acc_coes_combo, MC_COES),
    ...
    COMBO(r_med_esp_combo, MC_ESCP),    COMBO(r_med_cvi_combo, MC_CVI),   COMBO(r_med_oes_combo, MC_OESC),
    COMBO(r_med_soe_combo, MC_SOES),    COMBO(r_nav_cop_combo, MC_COPY),  COMBO(r_nav_ent_combo, KC_ENT),
    COMBO(r_nav_pdn_combo, KC_PGDN),    COMBO(r_nav_pup_combo, KC_PGUP),  COMBO(r_nav_pas_combo, MC_PAST),
    COMBO(r_num_bsp_combo, KC_BSPC),    COMBO(r_num_ent_combo, KC_ENT),   COMBO(r_nav_bsp_combo, KC_BSPC),
    COMBO(r_num_npd_combo, TG_NUM),     COMBO(l_num_tab_combo, KC_TAB),   COMBO(r_num_tab_combo, KC_TAB),
    COMBO(r_rai_bsp_combo, KC_BSPC),    COMBO(r_rai_deq_combo, MC_DEQ),   COMBO(r_rai_gt_combo, TD_GT),
    COMBO(r_rai_lt_combo, TD_LT),       COMBO(r_rai_ltgt_combo, MC_LTGT), COMBO(r_rai_neq_combo, MC_NEQ),
    COMBO(r_rai_dcol_combo, TD_DCOL),   COMBO(r_rai_sar_combo, TD_SARR),  COMBO(r_rom_bsp_combo, KC_BSPC),
    COMBO(r_rom_cop_combo, MC_COPY),    COMBO(r_rom_d_combo, KC_D),       COMBO(r_rom_p_combo, KC_P),
    COMBO(l_rom_ent_combo, KC_ENT),     COMBO(r_rom_ent_combo, KC_ENT),   COMBO(r_rom_npd_combo, TG_NUM),
    COMBO(r_rom_j_combo, KC_J),         COMBO(r_rom_pas_combo, MC_PAST),  COMBO(r_rom_sal_combo, MC_SELC),
    COMBO(r_rom_sco_combo, TD_SCLE),    COMBO(r_rom_scp_combo, MC_SCOP),  COMBO(r_rom_sent_combo, MC_SENT),
    COMBO(r_rom_spa_combo, MC_SPAS),    COMBO(l_rom_tab_combo, KC_TAB),   COMBO(r_rom_tab_combo, KC_TAB),
    COMBO(l_acc_ent_combo, KC_ENT),     COMBO(r_acc_ent_combo, KC_ENT),   COMBO(l_acc_tab_combo, KC_TAB),
    COMBO(r_acc_tab_combo, KC_TAB),     COMBO(r_rom_z_combo, KC_Z),       COMBO(r_rom_x_combo, KC_X),
    COMBO(r_rom_y_combo, KC_Y),         COMBO(r_sho_clo_combo, MC_CLOS),  COMBO(r_sho_dyn_combo, TD_MACR),
    COMBO(r_sho_joi_combo, MC_JOIN),    COMBO(r_sho_z0r_combo, MC_Z0R),   COMBO(secret_1_combo, MC_SECRET_1)};
