...
const uint16_t PROGMEM r_num_dlw_combo[] = {KC_7, KC_8, KC_9, COMBO_END};
...
const uint16_t PROGMEM l_num_tab_combo[] = {KC_7, KC_8, KC_9, COMBO_END};
...
const uint16_t PROGMEM l_sym_squo_combo[] = {KC_RCBR, KC_HASH, COMBO_END};
const uint16_t PROGMEM l_sym_dquo_combo[] = {KC_RPRN, KC_DQUO, COMBO_END};
const uint16_t PROGMEM l_sym_btic_combo[] = {KC_RBRC, KC_QUES, COMBO_END};

const uint16_t PROGMEM r_sym_circ_combo[] = {KC_COLN, KC_EQL, COMBO_END};
const uint16_t PROGMEM r_sym_dol_combo[]  = {KC_MINS, KC_PLUS, COMBO_END};
const uint16_t PROGMEM r_sym_til_combo[]  = {KC_EXLM, KC_AMPR, COMBO_END};
const uint16_t PROGMEM r_sym_sec_combo[]  = {KC_MINS, KC_PLUS, KC_SLSH, COMBO_END};
const uint16_t PROGMEM r_sym_eur_combo[]  = {KC_PLUS, KC_SLSH, COMBO_END};
...
const uint16_t PROGMEM r_acc_dlw_combo[]  = {MC_GV_A, MC_SQ_O, MC_SQ_U, COMBO_END};
...
const uint16_t PROGMEM r_rom_dlw_combo[]  = {KC_L, KC_O, KC_U, COMBO_END};
...
combo_t key_combos[] = {
    COMBO(l_sym_cur_combo, MC_CUR),   COMBO(l_sym_par_combo, TD_PARN),   COMBO(l_sym_bra_combo, MC_BRAC),
    COMBO(l_sym_squo_combo, KC_QUOT), COMBO(l_sym_dquo_combo, KC_DQUO),  COMBO(l_sym_btic_combo, KC_GRV),
    ...
    COMBO(r_sym_bsp_combo, KC_BSPC),  COMBO(r_low_com_combo, KC_COMM),   COMBO(r_num_com_combo, KC_COMM),
    COMBO(l_acc_esc_combo, MC_ESCC),  COMBO(l_acc_ja_combo, MC_JA),      COMBO(l_acc_ques_combo, MC_QUES),
    COMBO(l_acc_hash_combo, KC_HASH), COMBO(l_low_cur_combo, MC_CUR),    COMBO(l_low_bra_combo, MC_BRAC),
    COMBO(l_low_par_combo, TD_PARN),  COMBO(l_nav_esc_combo, MC_ESCC),   COMBO(l_num_esc_combo, MC_ESCC),
    COMBO(l_num_f10_combo, KC_F10),   COMBO(l_num_f11_combo, KC_F11),    COMBO(l_num_ent_combo, KC_ENT),
    ...
    COMBO(l_rai_f14_combo, KC_F14),   COMBO(l_rai_f15_combo, KC_F15),    COMBO(l_rom_cap_combo, TG_CAPS),
    COMBO(l_rom_esc_combo, MC_ESCC),  COMBO(l_rom_esv_combo, MC_ESAV),   COMBO(l_rom_fin_combo, MC_FIND),
    COMBO(l_rom_ful_combo, MC_FULL),  COMBO(l_rom_del_combo, KC_DEL),    COMBO(l_rom_k_combo, KC_K),
    ...
    COMBO(r_acc_dlw_combo, MC_DWOR),  COMBO(r_acc_cao_combo, MC_CAO),    COMBO(r_acc_coes_combo, MC_COES),
    ...
    COMBO(r_nav_ent_combo, KC_ENT),   COMBO(r_nav_pdn_combo, KC_PGDN),   COMBO(r_nav_pup_combo, KC_PGUP),
    COMBO(r_nav_pas_combo, MC_PAST),  COMBO(r_num_bsp_combo, KC_BSPC),   COMBO(r_num_dlw_combo, MC_DWOR),
    COMBO(r_num_ent_combo, KC_ENT),   COMBO(r_nav_bsp_combo, KC_BSPC),   COMBO(r_num_npd_combo, TG_NUM),
    COMBO(l_num_tab_combo, KC_TAB),   COMBO(r_num_tab_combo, KC_TAB),    COMBO(r_rai_bsp_combo, KC_BSPC),
    COMBO(r_rai_deq_combo, MC_DEQ),   COMBO(r_rai_gt_combo, TD_GT),      COMBO(r_rai_lt_combo, TD_LT),
    COMBO(r_rai_ltgt_combo, MC_LTGT), COMBO(r_rai_neq_combo, MC_NEQ),    COMBO(r_rai_dcol_combo, TD_DCOL),
    COMBO(r_rai_sar_combo, TD_SARR),  COMBO(r_rom_bsp_combo, KC_BSPC),   COMBO(r_rom_dlw_combo, MC_DWOR),
    COMBO(r_rom_cop_combo, MC_COPY),  COMBO(r_rom_d_combo, KC_D),        COMBO(l_rom_ent_combo, KC_ENT),
    ...
    COMBO(r_rom_zj_combo, TD_ZJ),     COMBO(r_rom_x_combo, KC_X),        COMBO(r_rom_y_combo, KC_Y),
    COMBO(r_sho_clo_combo, MC_CLOS),  COMBO(r_sho_dyn_combo, TD_MACR),   COMBO(r_sho_joi_combo, MC_JOIN),
    COMBO(r_sho_z0r_combo, MC_Z0R),   COMBO(secret_1_combo, MC_SECRET_1)};
