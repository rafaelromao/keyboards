...
const uint16_t PROGMEM l_num_f14_combo[] = {SF_F6, CT_F5, COMBO_END};
const uint16_t PROGMEM l_num_f15_combo[] = {AL_F3, KC_F2, COMBO_END};
...
const uint16_t PROGMEM l_rai_f13_combo[]  = {KC_F8, KC_F9, COMBO_END};
const uint16_t PROGMEM l_rai_f14_combo[]  = {KC_F5, KC_F6, COMBO_END};
const uint16_t PROGMEM l_rai_f15_combo[]  = {KC_F2, KC_F3, COMBO_END};
...
const uint16_t PROGMEM r_sho_sew_combo[] = {MC_SELL, MC_MODM, COMBO_END};
#ifndef SAVE_MEMORY
const uint16_t PROGMEM r_sho_com_combo[] = {QK_LEAD, MC_SWLE, COMBO_END};
#else
const uint16_t PROGMEM r_sho_com_combo[] = {XXXXXXX, MC_SWLE, COMBO_END};
#endif

// Alpha 2

const uint16_t PROGMEM l_acc_esc_combo[]  = {KC_Q, MC_QU, COMBO_END};
const uint16_t PROGMEM l_acc_lh_combo[]   = {KC_Z, KC_X, COMBO_END};
const uint16_t PROGMEM l_acc_cir_combo[]  = {MC_QU, KC_K, COMBO_END};
const uint16_t PROGMEM l_acc_dquo_combo[] = {KC_X, KC_W, COMBO_END};
const uint16_t PROGMEM l_acc_bti_combo[]  = {MC_SQ_C, KC_V, COMBO_END};
const uint16_t PROGMEM r_acc_ja_combo[]   = {KC_J, MC_SQ_C, COMBO_END};
const uint16_t PROGMEM r_acc_til_combo[]  = {MC_CR_A, MC_CR_O, COMBO_END};
const uint16_t PROGMEM r_acc_squo_combo[] = {MC_TL_A, MC_SQ_A, COMBO_END};
const uint16_t PROGMEM r_acc_dol_combo[]  = {MC_GV_A, MC_SQ_O, COMBO_END};
...
const uint16_t PROGMEM l_rom_z_combo[]   = {LSFTT_N, LCTLT_S, LGUIT_T, COMBO_END};
const uint16_t PROGMEM l_rom_j_combo[]   = {RALTT_F, KC_C, LALTT_P, COMBO_END};
const uint16_t PROGMEM l_rom_q_combo[]   = {KC_B, KC_M, KC_G, COMBO_END};
...
const uint16_t PROGMEM r_rom_p_combo[]    = {RALTT_H, TD_COMM, COMBO_END};
...
combo_t key_combos[] = {
    ...
    COMBO(l_num_f14_combo, KC_F14),   COMBO(l_num_f15_combo, KC_F15),   COMBO(l_num_sav_combo, MC_SAVE),
    COMBO(l_rom_esc_combo, MC_ESCC),  COMBO(l_rom_esv_combo, MC_ESAV),  COMBO(l_rom_fin_combo, MC_FIND),
    COMBO(l_rom_ful_combo, MC_FULL),  COMBO(l_rom_j_combo, KC_J),       COMBO(l_rom_k_combo, KC_K),
    COMBO(l_rom_cap_combo, TG_CAPS),  COMBO(l_rom_num_combo, TG_NUMC),  COMBO(l_rom_q_combo, KC_Q),
    COMBO(l_rom_v_combo, KC_V),       COMBO(l_rom_sav_combo, MC_SAVE),  COMBO(l_rom_und_combo, MC_UNDO),
    COMBO(l_rom_usc_combo, KC_UNDS),  COMBO(l_rom_w_combo, KC_W),       COMBO(l_rom_z_combo, KC_Z),
    COMBO(r_acc_ao_combo, MC_AO),     COMBO(r_acc_bsp_combo, KC_BSPC),  COMBO(r_acc_cao_combo, MC_CAO),
    COMBO(r_acc_coes_combo, MC_COES), COMBO(r_acc_ja_combo, MC_JA),     COMBO(l_acc_lh_combo, MC_LH),
    COMBO(l_acc_cir_combo, MC_CIRC),  COMBO(r_acc_til_combo, MC_TILD),  COMBO(r_acc_oe_combo, MC_OE),
    COMBO(r_acc_squo_combo, MC_SQUO), COMBO(l_acc_dquo_combo, MC_DQUO), COMBO(l_acc_bti_combo, MC_BTIC),
    COMBO(r_acc_dol_combo, KC_DLR),   COMBO(r_low_ent_combo, KC_ENT),   COMBO(r_low_sco_combo, TD_SCLE),
    COMBO(r_sho_clo_combo, MC_CLOS),  COMBO(l_sho_bui_combo, MC_BUID),  COMBO(l_sho_ref_combo, MC_REFC),
    COMBO(r_sho_sew_combo, MC_SELW),  COMBO(r_sho_com_combo, MC_COMT),  COMBO(r_sho_z0r_combo, MC_Z0R),
    COMBO(r_med_ent_combo, TD_EENT),  COMBO(r_nav_bsp_combo, KC_BSPC),  COMBO(r_nav_ent_combo, KC_ENT),
    COMBO(r_nav_cop_combo, MC_COPY),  COMBO(r_nav_pas_combo, MC_PAST),  COMBO(r_num_bsp_combo, KC_BSPC),
    COMBO(r_num_ent_combo, KC_ENT),   COMBO(r_num_npd_combo, TG_NUM),   COMBO(r_num_tab_combo, KC_TAB),
    COMBO(r_rai_bsp_combo, KC_BSPC),  COMBO(l_rai_f10_combo, KC_F10),   COMBO(l_rai_f11_combo, KC_F11),
    COMBO(l_rai_f12_combo, KC_F12),   COMBO(l_rai_f13_combo, KC_F13),   COMBO(l_rai_f14_combo, KC_F14),
    COMBO(l_rai_f15_combo, KC_F15),   COMBO(r_rai_deq_combo, MC_DEQ),   COMBO(r_rai_gt_combo, TD_GT),
    COMBO(r_rai_lt_combo, TD_LT),     COMBO(r_rai_ltgt_combo, MC_LTGT), COMBO(r_rai_neq_combo, MC_NEQ),
    COMBO(r_rai_sar_combo, TD_SARR),  COMBO(r_rom_bsp_combo, KC_BSPC),  COMBO(r_rom_cop_combo, MC_COPY),
    COMBO(r_rom_d_combo, KC_D),       COMBO(r_rom_ent_combo, KC_ENT),   COMBO(r_rom_npd_combo, TG_NUM),
    COMBO(r_rom_p_combo, KC_P),       COMBO(r_rom_pas_combo, MC_PAST),  COMBO(r_rom_sal_combo, MC_SELC),
    COMBO(r_rom_sco_combo, TD_SCLE),  COMBO(r_rom_tab_combo, KC_TAB),   COMBO(r_rom_v_combo, KC_V),
    COMBO(r_rom_x_combo, KC_X),       COMBO(r_rom_y_combo, KC_Y),       COMBO(secret_1_combo, MC_SECRET_1)};